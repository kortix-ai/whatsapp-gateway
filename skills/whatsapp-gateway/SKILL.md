...
Persisted state routes are `GET /v1/accounts/{accountId}/chats`, `GET /v1/accounts/{accountId}/contacts`, `GET /v1/accounts/{accountId}/groups`, `GET /v1/accounts/{accountId}/messages`, and `GET /v1/events`. To retrieve an attachment, call `GET /v1/accounts/{accountId}/messages/{messageId}/media`, which returns the decrypted image, video, audio, or document bytes with their original content-type (add `?download=1` to force a file download). Durable control routes are `GET /v1/baileys-actions`, `POST /v1/accounts/{accountId}/actions/{action}`, and `GET /v1/commands/{commandId}`.
